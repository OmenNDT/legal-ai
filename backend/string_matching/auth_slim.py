"""Minimal /auth/* blueprint for the String Matching service.

Schema dùng từ DB `legal_ai` (đã có):
  users(user_id UUID PK, email, password_hash, full_name, is_active, is_deleted, ...)

Endpoints:
  POST /auth/register         { name, email, password }       -> { user, token }
  POST /auth/login            { email, password }              -> { user, token }
  POST /auth/forgot-password  { email }                        -> { sent: true }   (chỉ verify email tồn tại)
  POST /auth/reset-password   { email, password }              -> { ok: true }     (passwordless theo y/c)
  GET  /auth/me                                               -> { user }
  DELETE /auth/me                                             -> { ok: true }

Token: JWT HS256, exp 24h. Secret từ env BDP_SM_JWT_SECRET hoặc fallback hằng số (dev).
"""
...
JWT_SECRET = os.environ.get(
    "BDP_SM_JWT_SECRET",
    "string-matching-dev-secret-change-me-9f3e7a1c2d4b6e8",
)
JWT_ALGO = "HS256"
TOKEN_TTL_HOURS = 24
...
@contextmanager
def _cur():
    assert _pool is not None, "auth_slim.init_pool() chưa được gọi"
    c = _pool.getconn()
    try:
...
auth_bp = Blueprint("auth_slim", __name__, url_prefix="/api/auth")
